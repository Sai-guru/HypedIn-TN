...
export default function GalleryAdminManagement() {
  ...
  const [galleryItems, setGalleryItems] = useState<GalleryItem[]>([
    {
      ...
      description:
        "Our recent food drive collected over 500 meals for families in need.",
      category: "Events",
    ...
    },
    {
      ...
      description:
        "Training our amazing volunteers for upcoming community projects.",
      category: "Volunteers",
    ...
    },
    {
      ...
      description:
        "Launching our new education initiative for underprivileged children.",
      category: "Impact",
  ...
  const [categories, setCategories] = useState<Category[]>([
    ...
    },
    {
      id: "2",
      name: "Volunteers",
      description: "Our amazing volunteer community",
      itemCount: 0,
    },
    {
      id: "3",
      name: "Impact",
      description: "Stories of positive change",
      itemCount: 0,
    },
    {
      id: "4",
  ...
  const API_URL = process.env.NEXT_PUBLIC_API_BASE_URL
    ? `${process.env.NEXT_PUBLIC_API_BASE_URL.replace(/\/+$/, "")}/api/gpage`
    : "http://localhost:5000/api/gpage";

  // Function to calculate item count for each category
  const calculateCategoryItemCounts = (
    items: GalleryItem[],
    cats: Category[]
  ) => {
    return cats.map((cat) => ({
  ...
  useEffect(() => {
    setCategories((prevCategories) =>
      calculateCategoryItemCounts(galleryItems, prevCategories)
    );
  }, [galleryItems]);
    ...
    try {
      ...
      if (res.data && Array.isArray(res.data)) {
        const mapped = res.data.map((item: any) => ({
          ...
          thumbnail:
            item.thumbnail || item.imageUrl || "/placeholder-image.jpg",
    ...
    try {
      ...
      const itemData = {
        ...
        tags: Array.isArray(newItem.tags)
          ? newItem.tags.filter((tag) => tag.trim())
          : [],
  ...
  const toggleFeatured = async (id: string) => {
    try {
      setError("");
      const response = await axios.put(
        `${API_URL}/items/${id}/toggle-featured`
      );
      if (response.data) {
        await fetchGalleryItems();
      }
  ...
  const toggleRotatingGallery = async (id: string) => {
    try {
      setError("");
      const response = await axios.put(
        `${API_URL}/items/${id}/toggle-rotating`
      );
      if (response.data) {
        await fetchGalleryItems();
      }
      ...
      <div className="p-6">
                    ...
                    <div className="ml-4 flex-1">
                      <div className="flex items-center justify-between">
                        <h3 className="font-medium text-gray-900 flex items-center">
                          {item.title}
                          {item.featured && (
                            <Star className="w-4 h-4 text-orange-500 ml-2 fill-current" />
                          )}
                          {item.rotatingGallery && (
                            <RotateCcw className="w-4 h-4 text-red-500 ml-2" />
                          )}
                        </h3>
                      </div>
        ...
        {activeTab === "rotating" && (
          <div className="bg-white rounded-lg shadow-sm border">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
              {galleryItems
                .filter((item) => item.rotatingGallery)
                .map((item) => (
                  <div
                    key={item.id}
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={(e) =>
                        setNewItem({
                          ...newItem,
                          rotatingGallery: e.target.checked,
                        })
                      }
                      className="mr-2"
                    />
