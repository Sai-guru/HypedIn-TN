...
export default function AdminLegalRightsSection() {
  const [activeTab, setActiveTab] = useState<
    "overview" | "rights" | "schemes" | "resources"
  >("overview");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [rRes, sRes, resRes] = await Promise.all([
          safeFetch("/api/legalsec/rights"),
          safeFetch("/api/legalsec/schemes"),
          safeFetch("/api/legalsec/resources"),
        ]);

        const r = rRes.success ? rRes.data : [];
        const s = sRes.success ? sRes.data : [];
        const res = resRes.success ? resRes.data : [];
  ...
  const handleFileUpload = async (
    ...
    try {
      const res = await safeFetch("/api/legalsec/upload-file", {
        method: "POST",
        body: formData,
        // do not set Content-Type, let browser set multipart boundary
      });

      if (!res.success) throw new Error(res.error || "File upload failed");
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (endpointType === "scheme")
        setSchemes((prev) => [...prev, newItem]);
      else if (endpointType === "resource")
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (activeTab === "rights") {
      ...
      endpoint = `${API_HOST}/api/legalsec/rights/${_id}`;
      ...
      endpoint = `${API_HOST}/api/legalsec/schemes/${_id}`;
      ...
      endpoint = `${API_HOST}/api/legalsec/resources/${_id}`;
    }
  ...
  const handleDelete = async (
    type: "right" | "scheme" | "resource",
    _id: string
  ) => {
    if (!window.confirm("Are you sure?")) return;

    const url = `${API_HOST}/api/legalsec/${type}s/${_id}`;
    try {
      const res = await fetch(url, { method: "DELETE" });
      if (!res.ok) throw new Error("Delete failed");

      if (type === "right")
        setRights((prev) => prev.filter((i) => i._id !== _id));
      if (type === "scheme")
        setSchemes((prev) => prev.filter((i) => i._id !== _id));
      if (type === "resource")
        setResources((prev) => prev.filter((i) => i._id !== _id));
    } catch (err) {
      console.error(err);
  ...
  const toggleStatus = async (
    ...
    const currentList =
      type === "right" ? rights : type === "scheme" ? schemes : resources;
    ...
    try {
      const res = await fetch(
        `${API_HOST}/api/legalsec/${type}s/${_id}/status`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
      ...
      const updatedItem = await res.json();
      if (type === "right")
        setRights((prev) => prev.map((i) => (i._id === _id ? updatedItem : i)));
      else if (type === "scheme")
        setSchemes((prev) =>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              Legal Rights & Schemes Management
            </h1>
            <p className="text-gray-600">
              Manage rights, government schemes, and downloadable resources
            </p>
          </div>
        </div>
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <div className="text-sm text-blue-600">
                    Create a new legal right entry
                  </div>
                </div>
              </button>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                        ...
                        <span>
                          Applications: {scheme.applications.toLocaleString()}
                        </span>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-red-700">
                              The current file will be deleted when you save.
                            </p>
                          </div>
                        </div>
                        <button
