...
const AdminLawFinder = () => {
  ...
  const [activeTab, setActiveTab] = useState<"laws" | "analytics" | "settings">(
    "laws"
  );
  ...
  const calculateAnalytics = (lawsData: Law[]): AnalyticsData => {
    ...
    const averageKeywordsPerLaw =
      Math.round(
        (lawsData.reduce((sum, law) => sum + law.keywords.length, 0) /
          totalLaws) *
          10
      ) / 10;
    ...
    const recentLaws = [...lawsData]
      .sort((a, b) => {
        const dateA = new Date(
          a.createdAt || a.updatedAt || "1970-01-01"
        ).getTime();
        const dateB = new Date(
          b.createdAt || b.updatedAt || "1970-01-01"
        ).getTime();
        return dateB - dateA;
      })
      .slice(0, 5)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch laws
        const lawRes = await axios.get(`${API_BASE}/find`);
        console.log("Laws fetched:", lawRes.data); // Debug log

        // Fetch settings
        const settingsRes = await axios.get(`${API_BASE}/find/set`);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const API_HOST = process.env.NEXT_PUBLIC_API_BASE_URL
        ...
        : "http://localhost:5000";
      const API_BASE = `${API_HOST}/api`;
      const res = await axios.put(`${API_BASE}/find/${editLaw._id}`, editLaw);
  ...
  const handleAddNewLaw = async () => {
    ...
    try {
      const API_HOST = process.env.NEXT_PUBLIC_API_BASE_URL
        ...
        : "http://localhost:5000";
      const API_BASE = `${API_HOST}/api`;
      const res = await axios.post(`${API_BASE}/find`, lawToAdd);
  ...
  const handleDeleteLaw = async (id: string) => {
    try {
      const API_HOST = process.env.NEXT_PUBLIC_API_BASE_URL
        ...
        : "http://localhost:5000";
      const API_BASE = `${API_HOST}/api`;
      await axios.delete(`${API_BASE}/find/${id}`);
  ...
  const saveSettings = async () => {
    try {
      ...
      const API_HOST = process.env.NEXT_PUBLIC_API_BASE_URL
        ...
        : "http://localhost:5000";
      const API_BASE = `${API_HOST}/api`;
      await axios.put(`${API_BASE}/find/set`, settings); // Ensure this is PUT
  ...
  const createBackup = () => {
    ...
    link.href = url;
    link.download = `lawfinder-backup-${
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600">
              Manage all laws and content in the LawFinder database
            </p>
          </div>
          <button
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <div className="text-xs text-gray-500">
                            {law.sections.length}{" "}
                            {law.sections.length === 1 ? "section" : "sections"}
                          </div>
                        </div>
                      </div>
                    </motion.div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={newLaw.act}
                        onChange={(e) =>
                          setNewLaw({ ...newLaw, act: e.target.value })
                        }
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setNewLaw({ ...newLaw, summary: e.target.value })
                        }
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setNewLaw({ ...newLaw, fullText: e.target.value })
                        }
                  ...
                  {searchResults.length > 0 ? (
                    ...
                    <div className="bg-white p-8 rounded-xl shadow-sm border text-center">
                      ...
                      <p className="text-gray-500">
                        No laws match your search query "{searchQuery}". Try
                        different keywords.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="p-6">
                    ...
                    {isEditing && editLaw ? (
                      <div className="space-y-4">
                        <div>
                          ...
                          <input
                            ...
                            value={editLaw.title}
                            onChange={(e) =>
                              setEditLaw({ ...editLaw, title: e.target.value })
                            }
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={editLaw.act}
                            onChange={(e) =>
                              setEditLaw({ ...editLaw, act: e.target.value })
                            }
                          />
                        </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          <h4 className="font-medium text-gray-900 mb-2">
                            Sections ({selectedLaw.sections.length})
                          </h4>
                          <div className="space-y-1">
                        ...
                        <div>
                          <h4 className="font-medium text-gray-900 mb-2">
                            Keywords ({selectedLaw.keywords.length})
                          </h4>
                          <div className="flex flex-wrap gap-2">
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      {analytics.averageKeywordsPerLaw}
                    </p>
                  </div>
                          ...
                          <div
                            ...
                            style={{
                              width: `${
                                (item.count /
                                  Math.max(
                                    ...analytics.mostCommonKeywords.map(
                                      (k) => k.count
                                    )
                                  )) *
                                100
