...
export default function CaseTrackerAdmin() {
  const [activeTab, setActiveTab] = useState<
    "cases" | "users" | "settings" | "analytics"
  >("cases");
  ...
  const [currentEditItem, setCurrentEditItem] = useState<Case | User | null>(
    null
  );
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (settingsRes && typeof settingsRes === "object") {
          ...
          showNotification(
            "error",
            "Received invalid settings data from server"
          );
        }
  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      const url = currentEditItem
        ? `${API}/users/${currentEditItem._id}`
        : `${API}/users`;
  ...
  const handleSaveCase = async () => {
    ...
    try {
      ...
      if (currentEditItem) {
        setCases((prev) =>
          prev.map((c) => (c._id === currentEditItem._id ? savedCase : c))
        );
  ...
  const handleDelete = async () => {
    ...
    try {
      if (deleteTarget.type === "case") {
        ...
        // Update frontend state
        setCases((prev) =>
          prev.filter((caseItem) => !deleteTarget.ids.includes(caseItem._id))
        );
        setSelectedCases([]);
        ...
        showNotification(
          "success",
          `${deleteTarget.ids.length} case(s) deleted successfully`
        );
        ...
        // Update frontend state
        setUsers((prev) =>
          prev.filter((user) => !deleteTarget.ids.includes(user.id))
        );
        setSelectedUsers([]);
        ...
        showNotification(
          "success",
          `${deleteTarget.ids.length} user(s) deleted successfully`
        );
      }
  ...
  const handleExportCases = () => {
    ...
    showNotification(
      "success",
      `${dataToExport.length} cases exported successfully`
    );
  };
  ...
  const handleExportUsers = () => {
    ...
    showNotification(
      "success",
      `${dataToExport.length} users exported successfully`
    );
  };
  ...
  const handleSaveSettings = async () => {
    setIsLoading(true);
    try {
      const res = await safeFetch("/api/lcase/settings", {
        method: "POST",
        headers: {
      ...
      await safeFetch("/api/lcase/logs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user: "Admin User",
  ...
  const handleBackupData = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `casetracker_backup_${new Date()
        .toISOString()
        .slice(0, 10)}.json`;
      a.click();
      window.URL.revokeObjectURL(url);
    ...
    <div className="min-h-screen bg-gray-50">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                              ...
                              <div className="flex items-center">
                                <div className="w-32 bg-gray-200 rounded-full h-2 mr-2">
                                  <div
                                    ...
                                    style={{
                                      width: `${
                                        (count / analytics.totalCases) * 100
                                      }%`,
                                    }}
                                  ></div>
                                </div>
                ...
                <div className="bg-white overflow-hidden shadow rounded-lg">
                  <div className="px-4 py-5 sm:p-6">
                    ...
                    <div className="mt-4">
                      <div className="space-y-3">
                        {Object.entries(analytics.roleCounts).map(
                          ([role, count]) => (
                            <div
                              key={role}
                              className="flex items-center justify-between"
                        ...
                        {activityLogs.slice(0, 10).map((log, index) => (
                          <li key={log._id}>
                            <div className="relative pb-8">
                              {index !==
                                activityLogs.slice(0, 10).length - 1 && (
                  ...
                  <div className="mt-6 space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-500">
                          Send email and in-app notifications for case updates
                        </p>
                      </div>
                      <div className="ml-4">
                        <button
                  ...
                  <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    ...
                    <div className="sm:col-span-3">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setCaseForm({ ...caseForm, judge: e.target.value })
                        }
                      />
                    </div>
                    <div className="sm:col-span-3">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              <div>
                ...
                <div className="mt-3 text-center sm:mt-5">
                  <h3 className="text-lg leading-6 font-medium text-gray-900">
                    Delete {deleteTarget.ids.length} {deleteTarget.type}
                    {deleteTarget.ids.length > 1 ? "s" : ""}?
                  </h3>
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      Are you sure you want to delete these {deleteTarget.type}
                      {deleteTarget.ids.length > 1 ? "s" : ""}? This action
                      cannot be undone.
                    </p>
                  </div>
                </div>
      ...
      {notification && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -20 }}
          className={`fixed top-4 right-4 z-50 rounded-md shadow-lg overflow-hidden max-w-sm w-full ${
