...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Backend uses 200 for both success and failure but returns `success: true` on success.
      if (!response.ok) {
      ...
      if (!data || data.success !== true) {
        setErrors({
          general: data?.message || data?.error || "Invalid credentials",
        });
        return;
      }

      // Prefer `accessToken` from backend; fall back to other names
      ...
      >
        ...
        <div className="backdrop-blur-xl bg-white/10 rounded-3xl shadow-2xl border border-white/20 p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-white/70">Welcome back, please sign in</p>
          </div>
