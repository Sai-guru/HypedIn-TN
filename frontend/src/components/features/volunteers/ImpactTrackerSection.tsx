...
const ImpactTrackerSection = () => {
  ...
  useEffect(() => {
    const fetchImpactData = async () => {
      try {
        setLoading(true);
        const result = await safeFetch("/api/vimpact");

        if (!result.success || !result.data) {
          throw new Error(result.error || "Failed to fetch data");
        }

        const data: ImpactData = result.data;
        setImpactData(data);
        setError(null);
      } catch (err) {
  ...
  const handleViewAllVolunteers = async () => {
    if (showVolunteers) {
    ...
    try {
      ...
      const response = await safeFetch<VolunteerRecord[]>("/api/vsignup");

      if (!response.success || !Array.isArray(response.data)) {
        throw new Error(response.error || "Unable to load volunteers");
      }
        ...
        >
          ...
          >
            ...
            <p className="text-4xl font-bold text-blue-600">
              {animatedStats.volunteersThisMonth}
            </p>
          ...
          >
            ...
            <p className="text-4xl font-bold text-purple-600">
              {animatedStats.totalHours.toLocaleString()}
            </p>
          ...
          >
            ...
            <p className="text-4xl font-bold text-amber-600">
              {animatedStats.livesImpacted.toLocaleString()}
            </p>
        ...
        >
          <div className="md:flex">
            <div className="md:w-1/2 p-8 bg-gradient-to-br from-indigo-800 to-purple-700 text-white">
              ...
              <p className="mb-6">
                See the difference your time can make. Move the slider to
                calculate the impact you could have.
              </p>
                ...
                <div>
                  <p className="text-gray-600 mb-2">
                    ...
                    <span className="font-bold text-indigo-700">
                      {userHours} {userHours === 1 ? "hour" : "hours"}
                    </span>{" "}
                    per month:
                  </p>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className="text-3xl font-bold text-purple-700">
                        {userImpact.people * 12}
                      </p>
                      <p className="text-sm text-gray-500">
                        People you could help annually
                      </p>
                    </div>
                  </div>
                </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {activeVolunteers.map((volunteer, index) => (
              ...
              >
                <div
                  className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-lg mr-4 ${getAvatarColor(
                    volunteer.name
                  )}`}
                >
          ...
          {showVolunteers && (
            <div className="mt-6 bg-white rounded-xl shadow-md p-6 border border-indigo-50">
              {volunteersLoading && (
                <div className="flex items-center justify-center text-indigo-600">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-indigo-600 mr-3"></div>
                  <span>Fetching volunteers...</span>
                </div>
              )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {volunteers.map((volunteer, index) => (
                      <div
                        ...
                        className="border rounded-lg p-4 bg-gradient-to-r from-indigo-50 to-white shadow-sm"
                      >
                        <p className="font-semibold text-gray-800">
                          {[volunteer.firstName, volunteer.lastName]
                            .filter(Boolean)
                            .join(" ") || "Unnamed Volunteer"}
                        </p>
