    ...
    const fetchData = async () => {
      try {
        ...
        const [testimonialsResult, settingsResult] = await Promise.all([
          safeFetch("/api/dtest"),
          safeFetch("/api/dtest/settings"),
        ]);

        if (!testimonialsResult.success || !testimonialsResult.data) {
          throw new Error(
            testimonialsResult.error || "Failed to fetch testimonials"
          );
        }
  ...
  useEffect(() => {
    ...
    if (scrollRef.current) {
      const testimonialCards =
        scrollRef.current.querySelectorAll(".testimonial-card");
      if (testimonialCards.length === 0) return;

      const totalWidth = Array.from(testimonialCards).reduce(
        (width, card) => width + card.clientWidth + 24,
        0
      );
      const scrollWidth =
        ...
        <div className="animate-pulse">
          ...
          <div className="flex space-x-6 overflow-hidden">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="flex-shrink-0 w-80 md:w-96 h-64 bg-white bg-opacity-10 rounded-xl"
              ></div>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto text-center py-12">
        <div className="bg-white bg-opacity-10 rounded-lg p-8">
          ...
          <p className="text-sm opacity-80">
            Check back later for donor stories.
          </p>
        </div>
      </div>
    );
      ...
      >
        {testimonials.map((testimonial, index) => (
          ...
          >
            ...
            <div className="mb-4">
              <svg
                className="w-8 h-8 text-purple-300 mb-2 opacity-50"
                fill="currentColor"
                viewBox="0 0 32 32"
              >
