  ...
  news: [
    {
      ...
      date: "2024-06-27",
      summary:
        "Key highlights from the Economic Survey 2024 including GDP growth projections and policy recommendations.",
      image: null,
    ...
    },
    {
      ...
      date: "2024-06-26",
      summary:
        "Latest updates on the implementation of National Education Policy 2020 across different states.",
      image: null,
...
export default function CurrentAffairsAdmin() {
  ...
  const [data, setData] = useState<any>(mockCurrentAffairsData);
    ...
    try {
      ...
      if (activeTab === "news") {
        ...
        if (modalType === "add") {
          ...
          setData((prev) => ({
            ...
            news: prev.news.map((n) =>
              n._id === savedItem._id ? savedItem : n
            ),
          }));
        }
      } else if (activeTab === "quiz") {
        ...
        if (modalType === "add") {
          ...
          setData((prev) => ({
            ...prev,
            quizzes: [...prev.quizzes, savedItem],
          }));
        } else {
          savedItem = await updateItem(editingItem._id, payload);
          setData((prev) => ({
            ...
            quizzes: prev.quizzes.map((q) =>
              q._id === savedItem._id ? savedItem : q
            ),
          }));
        }
      } else if (activeTab === "exams") {
        ...
        if (modalType === "add") {
          ...
          setData((prev) => ({
            ...prev,
            upcomingExams: [...prev.upcomingExams, savedItem],
          }));
        } else {
          savedItem = await updateItem(editingItem._id, payload);
          setData((prev) => ({
            ...prev,
            upcomingExams: prev.upcomingExams.map((e) =>
              e._id === savedItem._id ? savedItem : e
            ),
          }));
        }
      }
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      ...
      if (activeTab === "news") {
        setData((prev) => ({
          ...prev,
          news: prev.news.filter((n) => n._id !== itemId),
        }));
      } else if (activeTab === "quiz") {
        setData((prev) => ({
          ...prev,
          quizzes: prev.quizzes.filter((q) => q._id !== itemId),
        }));
      } else if (activeTab === "exams") {
        setData((prev) => ({
          ...prev,
          upcomingExams: prev.upcomingExams.filter((e) => e._id !== itemId),
        }));
      }
    } catch (err) {
          ...
          {filteredData().length === 0 && (
            <div className="p-12 text-center">
              <div className="text-slate-400 dark:text-slate-500 mb-4">
                {activeTab === "news" && (
                  <FaNewspaper size={48} className="mx-auto mb-4" />
                )}
                {activeTab === "quiz" && (
