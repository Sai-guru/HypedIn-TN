...
export default function CausesManagement() {
  ...
  const categories = [
    "All",
    ...Array.from(new Set(causes.map((cause) => cause.category))),
  ];
  ...
  const filteredCauses = causes.filter((cause) => {
    ...
    const matchesCategory =
      filterCategory === "All" || cause.category === filterCategory;
    const matchesStatus =
      filterStatus === "All" || cause.status === filterStatus;

    return matchesSearch && matchesCategory && matchesStatus;
  });

  const updateStats = (causesData: Cause[]) => {
    const totalRaised = causesData.reduce(
      (sum, cause) => sum + cause.raisedAmount,
      0
    );
    const avgProgress =
      causesData.length > 0
        ? Math.round(
            causesData.reduce((sum, cause) => sum + cause.progress, 0) /
              causesData.length
          )
        : 0;
    const activeCampaigns = causesData.filter(
    ...
    const fetchCauses = async () => {
      ...
      try {
        ...
        const processedCauses = res.data.map((cause: any) => ({
          ...cause,
          raisedAmount:
            typeof cause.raisedAmount === "string"
              ? parseInt(cause.raisedAmount.replace(/[$,]/g, ""))
              : cause.raisedAmount,
          goalAmount:
            typeof cause.goalAmount === "string"
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      if (axiosError.response?.status === 413) {
        ...
        alert(`Upload failed: ${axiosError.response.data.message}`);
      } else if (axiosError.code === "ECONNABORTED") {
        alert(
          "Upload timeout. Please try a smaller image or check your connection."
        );
  ...
  const handleSaveCause = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/causeList/${editingCause._id}`,
        updatedCause
      );
      ...
      const updatedCauses = causes.map((cause) =>
        cause._id === editingCause._id
          ? { ...updatedCause, _id: editingCause._id }
          : cause
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            Manage your causes page content and campaigns
          </p>
        </div>
        <Link
                        ...
                        <div className="flex items-center mb-1">
                          <div className="w-full bg-gray-200 rounded-full h-2 mr-2">
                            <div
                              ...
                              style={{
                                width: `${Math.min(cause.progress, 100)}%`,
                              }}
                            ></div>
                          </div>
            ...
            <div className="p-6 space-y-6">
                    ...
                    onChange={(e) => {
                      ...
                      setEditingCause((prev) =>
                        prev
                          ? {
                              ...prev,
                              raisedAmount: Math.max(0, numValue || 0),
                            }
                          : null
                      );
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setEditingCause((prev) =>
                        prev
                          ? { ...prev, goalAmount: Math.max(0, numValue || 0) }
                          : null
                      );
                    }}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              <div>
                    ...
                    <div
                      className={`h-3 rounded-full ${getProgressColor(
                        editingCause.goalAmount > 0
                          ? Math.min(
                              Math.round(
                                (editingCause.raisedAmount /
                                  editingCause.goalAmount) *
                                  100
                              ),
                              100
                            )
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Progress is automatically calculated based on raised/goal
                  amounts
                </p>
              </div>
            </div>
