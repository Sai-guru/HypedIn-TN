...
exports.getDashboard = async (req, res) => {
  try {
    ...
    // Keep the counters in sync with real volunteer submissions.
    const startOfMonth = new Date();
    startOfMonth.setDate(1);
    startOfMonth.setHours(0, 0, 0, 0);

    const volunteersThisMonth = await Volunteer.countDocuments({
      createdAt: { $gte: startOfMonth },
    });

    // Only update the field if the live count differs to avoid unnecessary writes.
    if (dashboard.stats.volunteersThisMonth !== volunteersThisMonth) {
      dashboard.stats.volunteersThisMonth = volunteersThisMonth;
      await dashboard.save();
    }
...
exports.deleteVolunteer = async (req, res) => {
  try {
    ...
    const index = dashboard.topVolunteers.findIndex(
      (v) => v._id.toString() === id
    );
    if (index === -1) {
      return res.status(404).json({ error: "Volunteer not found" });
