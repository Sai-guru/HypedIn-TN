...
const LawFinder = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch all laws
        const lawsResult = await safeFetch('/api/find');
        if (!lawsResult.success || !lawsResult.data) {
          throw new Error(lawsResult.error || 'Failed to fetch laws');
        }
        const laws = lawsResult.data;
        setAllLaws(laws);

        // Fetch settings
        const settingsResult = await safeFetch('/api/find/set');
  ...
  const fetchLawById = async (lawId: string) => {
    try {
      const result = await safeFetch(`/api/find/${lawId}`);
      if (!result.success) {
