...
export default function LegalRightsSection() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!rightsRes.success || !schemesRes.success || !resourcesRes.success) {
  ...
  const updateRightViews = async (rightId: string) => {
    try {
      ...
      if (right) {
        await safeFetch(`/api/legalsec/rights/${rightId}`, {
          method: 'PUT',
  ...
  const handleDownload = async (resourceId: string) => {
    try {
      ...
      if (resource) {
        ...
        await safeFetch(`/api/legalsec/resources/${resourceId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
