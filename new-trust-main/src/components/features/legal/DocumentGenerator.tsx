...
const DocumentGenerator = () => {
  ...
  const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || '/api/docu';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch templates
        const templatesRes = await safeFetch(`${API_BASE_URL}/templates`);
        if (!templatesRes.success) throw new Error(templatesRes.error || 'Failed to fetch templates');
        ...
        // Fetch settings
        const settingsRes = await safeFetch(`${API_BASE_URL}/settings`);
        if (!settingsRes.success) throw new Error(settingsRes.error || 'Failed to fetch settings');
  ...
  const updateTemplateUsage = async (templateId: string) => {
    try {
      const res = await safeFetch(`${API_BASE_URL}/templates/${templateId}/usage`, {
        method: 'POST',
        headers: {
      ...
      if (!res.success) {
        console.error('Failed to update template usage:', res.error);
      }
    } catch (err) {
