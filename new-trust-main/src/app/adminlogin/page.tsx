...
const handleOtpVerify = async (e) => {
  ...
  try {
    ...
    const response = await safeFetch('/api/auth/verify-otp', {
      method: 'POST',
      headers: {
    ...
    if (!result || (result && !result.success && !result.message)) {
      setErrors({ otp: (result && (result.error || result.message)) || 'OTP verification failed' });
      return;
    }
...
const handleResetPassword = async (e: React.FormEvent) => {
  ...
  try {
    setIsLoading(true);
    const r = await safeFetch('/api/auth/reset-password', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        resetToken,
    ...
    if (!r.success) throw new Error(r.error || r.data?.message || 'Reset failed');
