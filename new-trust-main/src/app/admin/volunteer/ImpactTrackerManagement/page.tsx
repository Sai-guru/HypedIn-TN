...
export default function AdminImpactTrackerPage() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const res = await safeFetch('/api/vimpact');
        if (!res.success) throw new Error(res.error || 'Failed to fetch impact data');
        const data = res.data;
  ...
  const handleStatsUpdate = async () => {
    try {
      const res = await safeFetch('/api/vimpact/stats', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(tempStats)
      });

      if (!res.success) throw new Error(res.error || 'Failed to update stats');
  ...
  const handleVolunteerUpdate = async (id: string) => {
    try {
      const res = await safeFetch(`/api/vimpact/volunteers/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(editingVolunteerData)
      });

      if (!res.success) throw new Error(res.error || 'Failed to update volunteer');
  ...
  const handleAddVolunteer = async () => {
    if (newVolunteer.name && newVolunteer.hours && newVolunteer.badge && newVolunteer.email && newVolunteer.status) {
      try {
        ...
        const res = await safeFetch('/api/vimpact/volunteers', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(volunteerToAdd)
        });

        if (!res.success) throw new Error(res.error || 'Failed to add volunteer');

        const addedVolunteer = res.data;
  ...
  const handleDeleteVolunteer = async (id: string) => {
    try {
      const res = await safeFetch(`/api/vimpact/volunteers/${id}`, { method: 'DELETE' });
      if (res.success) {
