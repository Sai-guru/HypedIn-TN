...
export default function AdminVolunteerPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await safeFetch('/api/vvolun/pending');
        if (res.success && res.data) {
          console.log('API Response:', res.data);
          console.log('First item structure:', (res.data as any)[0]);
          setApplications(res.data as VolunteerApplication[]);
        }
      } catch (err) {
  ...
  const handleApproveApplication = async (id: string) => {
    try {
      const res = await safeFetch(`/api/vvolun/${id}/approve`, { method: 'PATCH' });
      if (res.success) setApplications(prev => prev.map(app => app._id === id ? { ...app, status: 'approved' } : app));
    } catch (error) {
  ...
  const handleRejectApplication = async (id: string) => {
    try {
      const res = await safeFetch(`/api/vvolun/${id}/reject`, { method: 'PATCH' });
      if (res.success) setApplications(prev => prev.map(app => app._id === id ? { ...app, status: 'rejected' } : app));
    } catch (error) {
