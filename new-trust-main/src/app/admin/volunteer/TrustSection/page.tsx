...
export default function TrustSectionAdmin() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await safeFetch('/api/vtrust');
        const data = res.data;

        // Handle API response with fallback to defaults
        if (res.success && data && data.partners && Array.isArray(data.partners)) {
          setPartners(data.partners);
        }

        if (res.success && data && data.trustMetrics) {
  ...
  logo: '/images/partners/default.svg'
      ...
      try {
        const response = await safeFetch('/api/vtrust/partner', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ partner: partnerToAdd }),
        });
  ...
  const handleSavePartner = async () => {
    ...
    try {
      ...
      try {
        const response = await safeFetch(`/api/vtrust/partner/${editingPartner._id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ partner: editingPartner }),
        });
  ...
  const handleDeletePartner = async (_id: string) => {
    ...
    try {
      ...
      // Try API call first, but fallback to local update if it fails
      try {
        const response = await safeFetch(`/api/vtrust/partner/${_id}`, { method: 'DELETE' });
