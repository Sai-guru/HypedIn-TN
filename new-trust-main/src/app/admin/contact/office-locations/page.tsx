...
export default function AnimatedContactVisualAdmin() {
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        const analyticsRes = await safeFetch('/api/contact-visual/admin/analytics');
  ...
  const updateHighlights = async (newHighlights: Highlight[]) => {
    try {
      ...
      try {
        ...
        if (!res.success) throw new Error(res.error || 'Failed to update highlights');
  ...
  const updateVisualSettingsAPI = async (newSettings: VisualSettings) => {
    try {
      // Try API call, fallback to local state update for demo
      try {
        const res = await safeFetch('/api/contact-visual/admin/visual-settings', {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (!res.success) throw new Error(res.error || 'Failed to update visual settings');
  ...
  const addHighlight = async (title: string, description: string) => {
    try {
      ...
      try {
        ...
        if (!res.success) throw new Error(res.error || 'Failed to add highlight');
  ...
  const deleteHighlight = async (id: string) => {
    try {
      // Try API call, fallback to local state update for demo
      try {
        const res = await safeFetch(`/api/contact-visual/admin/highlights/${id}`, { method: 'DELETE' });

        if (!res.success) throw new Error(res.error || 'Failed to delete highlight');
      } catch (apiError) {
