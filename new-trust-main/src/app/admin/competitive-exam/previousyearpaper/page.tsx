...
export default function PreviousYearPapersAdmin() {
  ...
  const loadPapers = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await safeFetch('/api/epre');
      if (!res.success) {
        throw new Error(res.error || 'HTTP error while loading papers');
      }
      const data = res.data || [];
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await safeFetch('/api/epre/upload-image', {
        method: 'POST',
        body: form,
      });
  ...
  const handleAddPaper = async () => {
    ...
    try {
      ...
      if (!res.success) throw new Error(res.error || 'Failed to add paper');

      const savedPaper = res.data; // Will include _id from MongoDB
